...
public class ServiceActivity extends FragmentActivity implements OnMapReadyCallback {
        ...
        try {
            ...
            for (int i=0;i<jsonArray.length();i++) {
                ...
                LatLng latLng = new LatLng(Double.parseDouble(jsonObject.getString("Lat")),
                        Double.parseDouble(jsonObject.getString("Lag")));
    ...
    private  void  updateLatingLag() {
        try {
            EditLatLag editLatLag = new EditLatLag(ServiceActivity.this, loginStrings[0]);
            editLatLag.execute(Double.toString(userLatADouble),
                    Double.toString(userLngADouble));
            boolean b = Boolean.parseBoolean(editLatLag.get());
            Log.d("17febV2", "Result ==>"+b);
            editLatLag.cancel(true);
        } catch (Exception e) {
            Log.d("17feb","e update ==>"+ e.toString());
        }
    }
